					...
					<tr>
						...
						<td style="text-align: center;">
							<a href="/thumbnail/thumbdetail.jsp?ino=<%=vo.getBoardItemNo()%>&groupno=<%=vo.getBoardItemGroupNo()%>&pno=1">
								<img align="top" width="70px;" height="45px;" onmouseover="bigImg(this)" onmouseout="normalImg(this)" border="0" src="/itemImage/<%=vo.getBoardItemSrc() %>" alt="img" /><br /><font size="1px;"><%=vo.getBoardItemInfo() %></font>
							</a>
						</td>
						<% }%>
						...
						<%if(vo.getBoardNo() != vo.getBoardGroupNo()) {%> <i class="material-icons">&#xE5DA;</i> [re]<%}%>
							...
							if(!isAllFileNull) {%>
								<i class="material-icons">perm_media</i><%}if(isLogined) {if(_user.getNo() == 0) { %>
