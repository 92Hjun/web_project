					...
					if(!reviewList.isEmpty()) {
						...
						<tr>
							...
							<td class="tableTh2"><%=review.getReviewContents().replace("\n", "<br/>")%>
								<input type="hidden" id="<%=review.getReviewNo()%>" value="<%=review.getReviewNo()%>"/>
								<%if(_user.getNo() == review.getReviewUserNo()) { %>
								<a onclick="deleteReview(event);">
...
<script type="text/javascript">
	function commentCheck() {
 	...
 	function deleteReview(e) {
 		var reviewTrEL = e.target.previousSibling;
 		console.log(reviewTrEL);
 	}
</script>
</html>